...
import com.S.dailyread.Adapter.BannerPagerAdapter;
...
//    线程
    ...
    public  void init(){
        ...
        dotList.add(dot3);
        dotList.add(dot4);
        onBannerClickListener = new OnBannerClickListener() {        //添加图片的点击事件
            @Override
        ...
        runnable = new Runnable() {                  //此处是设置banner(横幅轮播页)的自动滚动的
            @Override
            public void run() {
...
//                只取前五张图片
            ...
            adapter.setOnBannerClickListener(onBannerClickListener);          //将此监听事件设置到本地的适配器中
